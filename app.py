...
def get_drive_service(credentials_dict):
    """使用真實使用者的 OAuth Token 建立 Google Drive 連線"""
...
with st.sidebar:
    ...
    if current_config:
        ...
        st.info("系統已記憶您的憑證。如需交接，請點擊下方按鈕刪除。")
        if st.button("🗑️ 刪除 API 設定", type="primary"):
            delete_config()
            st.rerun()
    else:
        st.warning("⚠️ 尚未綁定 Google Drive")
        st.markdown("請貼上真實帳號的 `token.json` 內容，以及**總資料夾 ID**。")
        
        token_json_input = st.text_area("1. 授權憑證 (貼上 token.json 的內容)", height=150, placeholder='{"token": "...", "refresh_token": "..."}')
        folder_id_input = st.text_input("2. Google Drive 「總資料夾」 ID", placeholder="例如：1n-wWYu0COFuMd0CczPdv...")
